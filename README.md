...
## tugas 6
1. Perbedaan synchronous vs asynchronous.Synchronous itu tiap kali kita kirim request, halaman di-refresh total dulu baru hasilnya kelihatan. Contohnya klik "Tambah Produk" lalu seluruh page nge-reload. Asynchronous (AJAX) kebalikannya: request dikirim di belakang layar, server balas data (biasanya JSON), terus kita cuma update bagian yang perlu di halaman-tanpa reload. Hasilnya terasa lebih cepat dan mulus.
2. Alur AJAX di Django.Di browser, aksi user (mis. submit form di modal) memicu fetch() ke URL Django. Di server, view Django nerima data, validasi, simpan ke database, lalu balikin JSON + status code. JavaScript baca JSON itu dan ngubah DOM: nutup modal, nambah kartu produk baru ke grid, atau nampilin toast. Semua kejadian tanpa pindah halaman.
3. Keuntungan AJAX dibanding render biasa.Yang paling berpengaruh: cepat dan hemat data karena kita hanya mengambil data mentah, bukan HTML satu halaman. UI juga bisa lebih interaktif: ada loading, error/empty state, modal, toast, dan tombol refresh list yang nggak bikin page kedip. Sisi minusnya, kita perlu sedikit lebih banyak JavaScript buat ngatur state dan error.
4. Keamanan Login/Register pakai AJAX.Tetap pakai CSRF (kirim X-CSRFToken), jalankan lewat HTTPS, dan manfaatkan django.contrib.auth buat autentikasi + session. Input harus divalidasi di server, pesan error jangan terlalu detail ("username atau password salah" sudah cukup), dan cookie session set HttpOnly, Secure, serta SameSite. Kalau perlu, tambah rate limiting biar nggak gampang dibruteforce. CORS cukup default (same-origin) kalau aplikasinya bukan publik API.
5. Dampak AJAX ke User Experience.Pengalaman pengguna jadi lebih responsif: aksi tambah/edit/hapus lebih mudah, posisi scroll tetap, modal nggak perlu buka-tutup halaman baru. Kita bisa kasih feedback jelas: tombol disabled saat submit, ada indikator loading, dan toast sukses/gagal. Intinya, alur kerja jadi lancar dan fokus user nggak pecah gara-gara reload.
